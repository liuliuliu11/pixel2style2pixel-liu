...
def run():
    ...
    for input_batch in tqdm(dataloader):
        with torch.no_grad():
            ...
            tic = time.time()
            result_batch, w_list = run_on_batch(input_cuda, net, opts)
            w_list_c = w_list.detach().cpu().numpy()
            path = test_opts.latent_dir + '/latent_{}'.format(global_i)
            np.savez(path, w_list_c)
            toc = time.time()
...
def run_on_batch(inputs, net, opts):
    if opts.latent_mask is None:
        result_batch, w_list = net(inputs, return_latents=True, randomize_noise=False)
